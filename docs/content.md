...
Sum every outgoing transaction's USD value over the trailing 90 days — transfers out, the outgoing side of swaps, and gas spent.
Divide that sum by 90 to get average daily burn.
Divide current USD balance by average daily burn to get runway in days.
...
Gas — total network fees paid across all outgoing transactions.
Top token by outflow — the single ERC-20/BEP-20/SPL token with the highest total USD value sent out, shown by symbol.
Top counterparty — the single address that received the highest total USD value, shown truncated (first 6 / last 4 characters) with a label indicating it's a wallet address, not a name Runway has independently verified.
